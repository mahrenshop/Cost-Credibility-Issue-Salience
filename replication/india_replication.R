...
colnames(data)[which(names(data) == "Q223")] <- "choice1"
colnames(data)[which(names(data) == "Q225")] <- "choice2"
colnames(data)[which(names(data) == "Q227")] <- "choice3"
colnames(data)[which(names(data) == "Q229")] <- "choice4"
colnames(data)[which(names(data) == "Q231")] <- "choice5"
colnames(data)[which(names(data) == "Q233")] <- "choice6"
...
in_amce <- plot(amces, legend_title = "Attribute", size = 2) +
  xlab("") +
  theme_minimal(base_size = 13)
in_amce
ggsave("in_amce.pdf", plot = in_amce, height = 7, width = 6)

# CONDITIONAL AMCE # INCOME

cand_control$income2 <- NA
cand_control$income2[cand_control$Q115 == "Less than \u20b960,000" | cand_control$Q115 == "\u20b960,000 - \u20b989,999"] <- "Low"
cand_control$income2[cand_control$Q115 == "\u20b91,20,000 - \u20b92,39,000" | cand_control$Q115 == "\u20b990,000 - \u20b91,19,000"] <- "Middle"
cand_control$income2[cand_control$Q115 == "\u20b92,40,000 - \u20b95,99,999" | cand_control$Q115 == "\u20b96,00,000 and over"] <- "High"
...
amces2 <- cj(cand_control, vote ~ issue + cost2 + effect2 + cred2 + gender + party,
             ...
             feature_labels = list(issue = "Issue", cost2 = "Costs", effect2 = "Effective",
                                   cred2 = "Credibility", gender = "Gender", party = "Party"))
in_amce_income <- plot(amces2, group = "income2", vline = 0, legend_title = "Income", size = 2) +
  xlab("") +
  theme_minimal(base_size = 13)
in_amce_income
ggsave("in_amce_income.pdf", plot = in_amce_income, height = 7, width = 6)
...
acies1 <- cj(cand_control, vote ~ issue + effect2 + cred2 + gender + party,
             ...
             feature_labels = list(issue = "Issue", effect2 = "Effective",
                                   cred2 = "Credibility", gender = "Gender", party = "Party"))
in_acie_cost <- plot(acies1, group = "cost2", vline = 0, legend_title = "Cost", size = 2, legend_pos = "bottom") +
  theme_minimal(base_size = 13)
in_acie_cost
ggsave("in_acie_cost.pdf", plot = in_acie_cost, height = 7, width = 6)
...
acies2 <- cj(cand_control, vote ~ issue + cost2 + cred2 + gender + party,
             ...
             feature_labels = list(issue = "Issue", cost2 = "Costs",
                                   cred2 = "Credibility", gender = "Gender", party = "Party"))
in_acie_effect <- plot(acies2, group = "effect2", vline = 0, legend_title = "Effective", size = 2) +
  theme_minimal(base_size = 13)
in_acie_effect
ggsave("in_acie_effect.pdf", plot = in_acie_effect, height = 7, width = 6)

### COMBINE ALL 4 PLOTS ###
in_cj_results <- ggarrange(in_amce, in_amce_income, in_acie_cost, in_acie_effect,
                           nrow = 2, ncol = 2, labels = c("A", "B", "C", "D"))
ggsave("in_cj_results.pdf", plot = in_cj_results, height = 9, width = 12)
...
budget$issue <- factor(budget$issue, levels = c("Education", "Economic Prosperity", "Domestic Security",
                                                "Environment", "Anti-Corruption", "Social Equality"))


in_budget <- ggplot(data = budget, aes(x = issue, y = mean)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(mean, 1)), vjust = 1.6, color = "white", size = 3.5) +
  ylab("%") + xlab("") +
  theme_minimal(base_size = 12)
in_budget
ggsave("in_budget.pdf", plot = in_budget, height = 7, width = 9)
...
stargazer(income_effect1, income_effect2, type = "text")
...
carryover <- cj(cand_control, vote ~ issue + cost2 + effect2 + cred2 + gender + party,
                ...
                feature_labels = list(issue = "Issue", cost2 = "Costs", effect2 = "Effective",
                                      cred2 = "Credibility", gender = "Gender", party = "Party"))
in_carry <- plot(carryover, group = "round", vline = 0, legend_title = "Round", size = 2) +
  theme_minimal(base_size = 13)
in_carry
...
profile_order <- cj(cand_control, vote ~ issue + cost2 + effect2 + cred2 + gender + party,
                    ...
                    feature_labels = list(issue = "Issue", cost2 = "Costs", effect2 = "Effective",
                                          cred2 = "Credibility", gender = "Gender", party = "Party"))
in_profile <- plot(profile_order, group = "cand", vline = 0, legend_title = "Candidate", size = 2) +
  theme_minimal(base_size = 13)
in_profile

## COMBINE INTO PLOT XY ##
in_tests <- ggarrange(in_carry, in_profile, ncol = 2)
ggsave("in_tests.pdf", plot = in_tests, height = 6, width = 10)

## Attention check
prop.table(table(cand_control$Q214 == 6)) # knowledge question with correct answer = 6
